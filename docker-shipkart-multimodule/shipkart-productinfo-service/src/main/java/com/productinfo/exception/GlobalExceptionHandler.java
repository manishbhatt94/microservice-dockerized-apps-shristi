...
public class GlobalExceptionHandler extends ResponseEntityExceptionHandler {
	...
	protected @Nullable ResponseEntity<Object> handleHttpMediaTypeNotSupported(HttpMediaTypeNotSupportedException ex,
		...
		ApiErrors apiErrors = ApiErrors
				.builder()
				.errorMessage(HttpStatus.UNSUPPORTED_MEDIA_TYPE.name())
				.exception(ex.getMessage())
				.timestamp(LocalDateTime.now())
				.status(status.value())
				.build();
	...
	protected @Nullable ResponseEntity<Object> handleMissingPathVariable(MissingPathVariableException ex,
		...
		ApiErrors apiErrors = ApiErrors
				.builder()
				.errorMessage(HttpStatus.valueOf(ex.getStatusCode().value()).name())
				.exception(ex.getMessage())
				.timestamp(LocalDateTime.now())
				.status(status.value())
				.build();
	...
	public ResponseEntity<Object> handleProductNotFoundException(ProductNotFoundException ex) {
		...
		ApiErrors apiErrors = ApiErrors
				.builder()
				.errorMessage("Product(s) with selected filter(s) not found")
				.exception(ex.getMessage())
				.timestamp(LocalDateTime.now())
				.status(statusCode)
				.build();
