...
@Service
public class ProductInfoServiceRCResilienceImpl implements IProductInfoService {
	...
	public ProductInfoServiceRCResilienceImpl(@Qualifier("loadBalancedRestClientBuilder") RestClient.Builder builder) {
		// Now 'builder' is the one with the LoadBalancer intercepter.
		// The service name 'product-catalog' is used as the base URL.
		this.restClient = builder.baseUrl("http://product-catalog:8081/catalog-service/v1").build();
	}

	@Override
	@CircuitBreaker(name = "infoServiceCircuitBreaker", fallbackMethod = "getByIdFallback")
	...
	public Product getByIdFallback(int productId, Exception e) {
		System.out.println("getByIdFallback - Received Exception: " + e);
		// return "product-catalog is down";
		return new Product();
	}
